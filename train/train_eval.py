...
from layoutlm import LayoutlmConfig, LayoutlmForTokenClassification, FunsdDataset
from layoutlm.modeling.optimization import AdamW, get_linear_schedule_with_warmup
...
def train(  # noqa C901
    ...
    optimizer = AdamW(
        optimizer_grouped_parameters, lr=float(args.learning_rate), eps=float(args.adam_epsilon)
    )
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=float(args.warmup_steps), num_training_steps=t_total
    )
    if args.fp16:
        try:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    int(args.local_rank) in [-1, 0]
                    and int(args.logging_steps) > 0
                    and global_step % int(args.logging_steps) == 0
                ):
                    # Log metrics
                    if (
